...
async def seeded_user(reset_engine):
    ...
    if migration_url:
        ...
        async with mig_engine.begin() as conn:
            await conn.execute(
                text(
                    "DELETE FROM custodian_logged_items WHERE user_id IN "
            ...
            await conn.execute(
                text(
                    "DELETE FROM importance_signals WHERE user_id IN "
                    "(SELECT id FROM users WHERE email = :e)"
                ),
                {"e": email},
            )
            ...
            await conn.execute(
                text(
                    "DELETE FROM planetary_nodes WHERE user_id IN "
                    "(SELECT id FROM users WHERE email = :e)"
                ),
                {"e": email},
            )
            ...
            await conn.execute(
                text(
                    "DELETE FROM concept_candidates WHERE user_id IN "
                    "(SELECT id FROM users WHERE email = :e)"
                ),
                {"e": email},
            )
            ...
            await conn.execute(
                text(
                    "DELETE FROM fragments WHERE user_id IN "
                    "(SELECT id FROM users WHERE email = :e)"
                ),
                {"e": email},
            )
