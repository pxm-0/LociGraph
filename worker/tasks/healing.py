"""Shared tuning + helper for dramatiq's on_retry_exhausted self-healing hooks.
...
def next_heal_generation(original_message: dict[str, Any]) -> int | None:
    """Next heal_generation to send with, or None once the cap is hit."""
    heal_generation = original_message["options"].get("heal_generation", 0)
    return None if heal_generation >= MAX_HEAL_GENERATIONS else heal_generation + 1
